...
the RStudio window now lists a data set called `arbuthnot` that has 82 observations on 3 variables. As you interact with R, you will create a series of objects. 
...
```{r names-data}
names(arbuthnot)
n_anos_unicos <- length(unique(arbuthnot$year))
print(n_anos_unicos)

arbuthnot$girls

arbuthnotgirls <- arbuthnot[, c("year", "girls")]


library(dplyr)

arbuthnotgirls2 <- arbuthnot %>%
  select(year, girls)
...
lot like functions from math cwlass; that is, invoking R commands means supplying
...
http://docs.ggplot2.org/current/. The best (and easiest) way to learn the syntax is to take a look at the sample plots provided on that page, and modify the code 
...
plot(present$year, present$prop_boys,
     ...
     main = "Proportion of Boys Born in the US Over Time")


lines(lowess(present$year, present$prop_boys), col = "red", lwd = 2)
legend("topright", legend = c("Proportion", "Smoothed Trend"),
...
plot(present$year, present$prop_boy_girl,
     ...
     main = "Boy-to-Girl Birth Ratio Over Time")

max_index <- which.max(present$total)
year_max_births <- present$year[max_index]
