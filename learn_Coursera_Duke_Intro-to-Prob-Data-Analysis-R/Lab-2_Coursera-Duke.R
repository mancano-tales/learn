...
We received the warning that we are wunning an old version of R, so we
might want to update ir
...
nycflights %>%
  group_by(month) %>%
  summarise(mean_dd = median(dep_delay), sd_dd = sd(dep_delay), n = n())
```
...
currently found group are on time - sum(dep_type == "on timeâ€) - and
...
nycflights %>% mutate( avg_speed = distance*60/air_time) %>% 
group_by(tailnum) %>%
...
9.  Make a scatterplot of `avg_speed` vs. `distance`. Which of the
    ...
    <li>There is an overall postive association between distance and
    average speed.</li>
...
p <- ggplot(df, aes(x = distance, y = avg_speed, text = tailnum)) +
  geom_point(alpha = 0.6) +
  labs(
    title = "Relação entre Velocidade Média e Distância",
    x = "Distância (milhas)",
    y = "Velocidade Média (mph)"
  ) +
  theme_minimal()
...
result <- nycflights %>%
  ...
  select(fraction_on_time_arrival = percent)

result$fraction_on_time_arrival
